...
from pre_processing import PreProcessingTrain
...
class Train(ABC):
    def __init__(self):
        ...
        self.X_val = None
        self.y_val = None
        self.pre_processing = PreProcessingTrain().pre_processing()
...
class Train_NeuralNetwork(Train):
    def train(self):
...
class Train_GradientBoostingRegressor(Train):
    def train(self):
        if os.path.exists('database/train_database.pkl'):
            with open('database/train_database.pkl', 'rb') as file:
                self.X_training, self.X_val, self.y_training, self.y_val = pickle.load(file)
...
class Train_RandomForestRegressor(Train):
    def train(self):
